package day03_ifStatement;

import java.sql.SQLOutput;
import java.util.Scanner;

public class C01_Scanner {
    public static void main(String[] args) {
         /*  Problem Tanımı
       Bir işçinin işi bitirme süresini ve toplam işçi sayısını alarak, işin bitme süresini
       hesaplayan kodu yazınız.
       Örneğin, Bir işçi bir işi 10 günde yapabilmektedir.
       Buna göre 2 işçi aynı işi kaç günde yapar?
       Örnek Ekran Çıktısı
        Bir işçi işi kaç günde bitirmektedir? 10
        Toplam kaç işçi çalışacak? 2
       İşin bitme süresi 5 gündür.
   */
        //1.YOL BENİM YAPTIGIM

        Scanner scan = new Scanner(System.in);
        System.out.println("Kaç işçi çalışacak");
        int işçi =scan.nextInt();
        int iş=100;
        int işçiKapasite =10;
        int sure=iş/(işçi*işçiKapasite);
        System.out.println("*******SONUÇ*******");
        System.out.println(sure+" "+"iş günü");


       //2.YOL HOCANIN YAPTIGI

        System.out.println("lütfen işçi sayısını giriniz  :" );
        int isciSayisi= scan.nextInt();
        System.out.println("lütfen ishacmini giriniz  :");
        double isHacmi=scan.nextDouble();

        double bitirmeSuresi= (isHacmi/isciSayisi);
        System.out.println("işi bitirme süresi   :"+bitirmeSuresi);


    }

}
